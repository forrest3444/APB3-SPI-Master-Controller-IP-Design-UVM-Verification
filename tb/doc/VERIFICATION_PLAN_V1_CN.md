...
- APB3-style always-ready 访问和非法地址行为
...
| 接口 | 关键语义 |
| --- | --- |
| APB | 12-bit 地址、32-bit 数据、`PREADY=1`、`PSLVERR=0` |
...
- 未映射或非对齐地址读取返回 0，写入无副作用，`PSLVERR` 保持 0。
...
| OBJ-APB | APB 协议 | setup/access、always-ready、无 error、背靠背访问 |
...
| F-REG-05 | 非法及非对齐地址读零、写无副作用 | P0 | TC-REG-06 |
...
| `apb_reg_semantics_test` | TC-REG-01/04/05/06/07 | 合并复位、WO/RO、非法地址、VERSION |
...
| `irq_stress_test` | TC-IRQ-01/02/03/04/05 | 多 IRQ 源、mask 切换、clear、软件复位 |
...
| 场景 | 期望结果 |
| --- | --- |
| 非对齐地址 `0x01/0x03/0x31` | 读 0、写无副作用、PSLVERR=0 |
| enable=0 时 start | 无传输、无 underflow |
| tx_en=rx_en=0 时 start | 无传输、无 underflow |
| tx_en=0、rx_en=1 | MOSI=0x00，接收一帧；cont=1 也不自动续帧 |
| tx_en=1、rx_en=0 | 正常发送，RX FIFO 不增加 |
| busy 期间 start | 当前帧不受影响，不额外启动帧 |
| start 与 soft_reset 同次写入 | reset 优先，不启动帧 |
| event 与 IRQ_CLEAR 同周期 | sticky 位保持清零 |
| cont=0 多帧 | 每次等待 IDLE 后 start，每帧独立 CS 窗口 |
| 冷复位输出 | CS=1、MOSI=0、SCLK=CPOL reset、irq=0，无 X/Z |
...
| APB no-error | access 时 PSLVERR=0 |
...
- 每日执行 `tb/tests/` 下除 `apb_spi_base_test` 外的全部 11 个可执行测试。
