...
| VTR-CODE-007 | 代码覆盖率审查 | `spi_ctrl` 行覆盖率，`tb_top.dut.u_spi_ctrl`，约 line 183 | 主 FSM case 语句中的 `default` 分支 | 不可达 — 防御性 | 添加代码覆盖率 waiver | `tb/cov_waivers/dve_waivers.el` | Waived | `state_q` 是 2-bit `spi_state_e` 枚举，覆盖全部 4 个值 {IDLE, LOAD, SHIFT, FRAME_DONE}，每个都有显式 case 分支。FSM 断言保证状态不会逃逸到非法编码。`default` 分支仅为综合安全的防御性写法。 |
| VTR-CODE-008 | 代码覆盖率审查 | `apb_reg_block` condition coverage，`tb_top.dut.u_apb_reg_block` | `psel && penable && !legal_addr` 向量 `011`；`psel && penable && pwrite` 向量 `011`（line 63）；`psel && penable && !pwrite` 向量 `011`（line 64） | 协议负向 / raw driver 可达 | 在 `pslverr_test` 中添加 raw APB driver override 场景 | `pslverr_test` / `agent/apb_agent/apb_raw_driver.sv` / `agent/apb_agent/apb_raw_trans.sv` | Verified | 三条同源：根因均为 `psel=0, penable=1`，合规 APB master 从不驱动此组合。场景发出两笔 raw transaction：写 (`pwrite=1`) 和读 (`pwrite=0`)，每笔均以 `psel=0, penable=1, paddr=0x031` 驱动。编译需 `ASSERT=0`，因场景违反断言 `PENABLE -> PSEL`。协议负向 raw-driver 专项场景，不进入默认 ASSERT=1 回归。 |
| VTR-TOG-001 | 代码覆盖率审查 | `apb_reg_block` toggle coverage，`tb_top.dut.u_apb_reg_block`，`prdata[31:17]` | 0→1 翻转从未命中 | 结构性 — 设计常量 | 添加 toggle 覆盖率 waiver | `tb/cov_waivers/dve_waivers.el` | Waived | `prdata[31:17]` 来源：所有寄存器 reserved 位（固定 0），VERSION major 位（固定 `16'h0001`，bit 16=1，bits 31:17=0）。设计中无逻辑驱动这些位为 1。 |
| VTR-TOG-002 | 代码覆盖率审查 | `apb_reg_block` toggle coverage，`tb_top.dut.u_apb_reg_block`，`prdata[15:9]` | 0→1 翻转从未命中 | 结构性 — 设计常量 | 添加 toggle 覆盖率 waiver | `tb/cov_waivers/dve_waivers.el` | Waived | `prdata[15:9]` 来源：STAT[8] rx_overflow_pending（可翻转，已在其他地方覆盖）；其余 bits 15:9 均为 reserved 固定 0。无逻辑驱动 bits[15:9]（不含 bit 8）为 1。 |
